...
const renderListType = () => {
	return taskTypes.forEach(type => {
		const btnType = buttonGen(``, type);

		btnType.addEventListener('click', function () {
			const newArray = tasks.filter(taskSingle => taskSingle.category === this.id)
			console.log(newArray.length ? newArray : 'There are no tasks of this category')
		})
		filtersElement.append(btnType);
	})
}
...
const addDeleteFunctionality = () => {
    ...
    deleteIcons.forEach(icon => {
        icon.addEventListener('click', function() {
            const cardToDelete = this.parentElement.parentElement;
            cardToDelete.remove();
        });
    });
};
